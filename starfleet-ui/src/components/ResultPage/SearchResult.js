...
export default function SearchResult(props) {
    ...
    useEffect(() => {
        axios({
            ...
            if (searchResults && searchResults.length > 0 && !searchResults.errors) {
    ...
    function shipDetailLayout() {
        ...
        return (
            <>
                {
                    shipKeys.filter(item => item !== "image").map((entry) => {
                        ...
                        return (
                            <div className="flex mx-auto my-8 w-full text-left justify-around" key={entry}>
                                <div className="p-1 w-1/3 text-yellow-200 px-10 align-middle items-center">{label}</div>
                                <div className="p-1 w-1/3 bg-white text-gray-600 rounded-md break-words px-10 text-center align-middle my-auto text-lg">{shipDetails[entry]}</div>
                            </div>
                        )
                    })
                }
    ...
    return (
        <>
            <div className="w-3/4 mx-auto my-6 border border-gray-600 rounded-md">
                <div className="search-header bg-yellow-500 text-gray-700 p-2 rounded-t-md text-4xl text-center">{shipDetails.name}</div>
                <div className="flex">
                    <div className="mx-10 my-20 rounded" style={{
